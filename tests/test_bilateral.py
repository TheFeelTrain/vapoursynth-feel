...
def test_bilateral_matches_reference(noise_gray):
...
def test_bilateral_16bit_matches_reference(noise_16bit):
...
def test_bilateral_sigma_sweep_matches_reference(noise_gray, sigma_spatial, sigma_color, tol):
...
def test_bilateral_radius_sweep_matches_reference(noise_gray, radius):
...
def test_bilateral_shader_variants_match_vszipcl(noise_gray, use_shared_memory):
...
@pytest.mark.parametrize("use_shared_memory", [True, False], ids=["shared", "plain"])
def test_bilateral_yuv_matches_reference(noise_gray, use_shared_memory):
...
def test_bilateral_ref_clip_matches_reference(noise_gray):

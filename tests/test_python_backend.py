...
that the package imports and its duck-typed backends run through the
...
def _backend(name):
    return getattr(vsfeel.Backend, name)


def test_backends_resolve():
    assert vsfeel.Backend.Bilateral.resolve() is vsfeel.Backend.Bilateral
    assert vsfeel.Backend.NLMeans.resolve() is vsfeel.Backend.NLMeans


def test_bilateral_runs_via_jetpack(noise_gray):
    out = bilateral(noise_gray, sigmaS=3.0, sigmaR=0.02, backend=_backend("Bilateral"))
    assert np.isfinite(frame_to_ndarray(out.get_frame(0))).all()


def test_nl_means_runs_via_jetpack(noise_gray):
    out = nl_means(noise_gray, h=1.2, tr=1, a=2, s=4, backend=_backend("NLMeans"))
    assert np.isfinite(frame_to_ndarray(out.get_frame(0))).all()


def test_gauss_blur_runs_via_jetpack(noise_gray):
    out = gauss_blur(noise_gray, 1.5, backend=_backend("GaussBlur"))
    assert np.isfinite(frame_to_ndarray(out.get_frame(0))).all()


def test_bm3d_runs_via_jetpack(noise_gray):
    out = bm3d(noise_gray, 0.7, tr=2, profile=bm3d.Profile.FAST, backend=_backend("BM3D"))
    assert np.isfinite(frame_to_ndarray(out.get_frame(0))).all()


def test_dfttest_runs_via_jetpack(noise_gray):
    dft = DFTTest(noise_gray, backend=_backend("DFTTest"))
